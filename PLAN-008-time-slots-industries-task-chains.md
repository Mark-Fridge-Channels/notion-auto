...
按时间区间（左闭右开、本地时区）选择当前行业，每个行业有独立 Notion Portal URL 与任务链；任务仅「输入内容 + 执行次数」，新会话/换模型为行业级「每 N/M 次」。7×24 无限跑直到用户停止；同一区间内任务链跑完立刻循环；恢复按当前时间对应行业从任务 1 开始。仅保留此一种模式，配置替代现有扁平 params。
