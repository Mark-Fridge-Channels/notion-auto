...
- **Dashboard「全部启动」错峰**：env `NOTION_AUTO_STARTUP_STAGGER_MS`（默认 5000ms）控制相邻账号的启动间隔，避免同一秒 N 个 Chromium 一起抢带宽 / CPU / swap，引发 Notion SPA 初刷互相拖累、`page.goto` 30s 超时。设为 `0` 回到旧行为（同时启动）。
