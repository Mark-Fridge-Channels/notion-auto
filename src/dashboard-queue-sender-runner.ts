...
export type QueueSenderRunStatus = "idle" | "running";

export interface QueueSenderRunLog {
  id: number;
  startTime: number;
  endTime: number | null;
  lines: string[];
...
let runLogs: QueueSenderRunLog[] = [];
let currentRunLog: QueueSenderRunLog | null = null;
...
export function getQueueSenderStatus(): QueueSenderRunStatus {
...
export function startQueueSender(): void {
  ...
  if (process.platform === "win32") {
    opts.shell = true;
    const args = ["tsx", "src/queue-sender.ts"];
    const fullCmd = "npx " + args.map(escapeArgForWindowsCmd).join(" ");
    child = spawn(fullCmd, opts);
  } else {
    child = spawn("npx", ["tsx", "src/queue-sender.ts"], opts);
  }
...
export function getQueueSenderRunLogs(n: number = MAX_RUN_LOGS): QueueSenderRunLog[] {
